...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  if (
    ...
    urlObj.search = params.toString()
    return urlObj.toString()
  } else if (image.indexOf('https://https://flt6-imgs.oss-cn-beijing.aliyuncs.com/') === 0) {
    // 此处还可以添加您的自定义图传的封面图压缩参数。
  // .e.g
    params.set('x-oss-process', 'image/resize,m_lfit,w_{}/watermark,image_aW1ncy9mbHRfbWFpbi5wbmc_eC1vc3MtcHJvY2Vzcz1pbWFnZS9yZXNpemUsUF8yNA'.format(width))
    urlObj.search = params.toString()
    return urlObj.toString()
  }
