...
const mapImgUrl = (img, block, type = 'block', needCompress = true) => {
  ...
  ret.replace("x-oss-process=init","x-oss-process=style/init")
  ret.replace("flt6-imgs.oss-cn-beijing.aliyuncs.com","oss.flt6.top")
  ...
  if (ret.includes("/images/page-cover/")||ret.includes("unsplash.com"))
    ret = BLOG.RANDOM_IMAGE_URL
...
const compressImage = (image, width, quality = 50, fmt = 'webp') => {
  ...
  if (image.includes(".gif")) return image
  if (image.includes(".webp")) return image
  ...
  if (
    ...
    urlObj.search = params.toString()
    return urlObj.toString()
  } else if (image.indexOf('https://oss.flt6.top/') === 0 && image.indexOf('.webp') !== 0 && image.indexOf('.gif') !== 0) {
    // 此处还可以添加您的自定义图传的封面图压缩参数。
  // .e.g
    params.set('x-oss-process', `image/resize,m_lfit,w_${width}`)
    urlObj.search = params.toString()
    return urlObj.toString()
  }
